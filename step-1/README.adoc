...
TIP: 由Vert.x项目支持的SQL数据库模块目前除了传递SQL查询之外还没有提供任何东西（例如，对象关系映射器），因为它们专注于提供对数据库的异步访问。 然而，没有任何东西禁止使用 https://github.com/vert-x3/vertx-awesome[来自社区的更高级的模块]，我们特别推荐成熟的项目，比如 https://github.com/jklingsporn/vertx-jooq[this jOOq generator for Vert.x] 或 https://github.com/BraintagsGmbH/vertx-pojo-mapper[this POJO mapper]。
