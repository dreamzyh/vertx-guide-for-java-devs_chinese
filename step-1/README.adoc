...
有 start` 和 `stop` 两种形式的方法：一个没有参数，一个有 _future_ 的对象引用。 无参数变体意味着verticle初始化或管家阶段总是成功，除非抛出异常。 具有未来对象的变体提供了更细粒度的方法，最终表明操作成功与否。 事实上，一些初始化或清理代码可能需要异步操作，所以通过未来的对象进行报告自然适合异步的习惯用法。There are 2 forms of `start` (and `stop`) methods: 1 with no argument and 1 with a _future_ object reference.
