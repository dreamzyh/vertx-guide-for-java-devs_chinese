...
本指南选择使用 https://maven.apache.org[Apache Maven]作为构建工具，主要是因为它与主要的集成开发环境很好地结合在一起。 您可以同样使用另一个构建工具，如 https://gradle.org/[Gradle]。This guide makes the choice of using https://maven.apache.org[Apache Maven] as the build tool, primarily because it is very well integrated with the major integrated development environments.
