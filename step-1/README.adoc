...
Vert.x核心API基于回调来通知异步事件。 经验丰富的开发人员自然会认为，这为所谓的“回调地狱”打开了大门，其中多层次的嵌套回调使得代码难以理解，如这个虚构的代码所示：The Vert.x core APIs are based on callbacks to notify of asynchronous events.
